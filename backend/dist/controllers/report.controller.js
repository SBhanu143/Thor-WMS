...
class ReportController {
    ...
    exportReport = async (req, res) => {
        try {
            const type = req.query.type; // 'inventory' | 'activity' | 'audits'
            const format = req.query.format; // 'pdf' | 'excel' | 'csv' | 'json'
            if (!type || !format) {
    ...
    async generateExcel(data, type, title) {
    ...
    generatePDF(doc, data, type, title) {
        // Colors
        ...
        if (type === 'inventory') {
            ...
            for (const item of data) {
                if (doc.y > 700)
                    doc.addPage();
                y = doc.y;
                this.drawTableRow(doc, y, [item.name, item.sku, item.barcode, String(item.quantity), item.bin_id || 'Unassigned'], colWidths);
                doc.moveDown(0.5);
    ...
    drawTableRow(doc, y, textList, colWidths) {
        let currentX = 50;
